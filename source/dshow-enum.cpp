		       ...
		       EnumDeviceCallback callback, void *param)
{
    ...
    if (deviceName.bstrVal && type == CLSID_VideoInputDeviceCategory)
        if (wcsstr(deviceName.bstrVal, L"VSeeFaceCamera") != nullptr || wcsstr(deviceName.bstrVal, L"VTubeStudioCam") != nullptr || wcsstr(deviceName.bstrVal, L"VMC_Camera") != nullptr || wcsstr(deviceName.bstrVal, L"3tene Screen Capture") != nullptr || wcsstr(deviceName.bstrVal, L"Unity Video Capture") != nullptr)
            return true;
